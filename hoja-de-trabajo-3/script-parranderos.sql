...
SELECT F.ID_BEBEDOR, BEB.NOMBRE, COUNT(F.ID_BEBEDOR) AS NUM_VISITAS
FROM PARRANDEROS.FRECUENTAN F
INNER JOIN PARRANDEROS.BEBEDORES BEB ON F.ID_BEBEDOR = BEB.ID
WHERE BEB.ID IN (
    SELECT DISTINCT G.ID_BEBEDOR
    FROM PARRANDEROS.GUSTAN G
    WHERE G.ID_BEBEDOR NOT IN (
        SELECT DISTINCT G.ID_BEBEDOR
        FROM PARRANDEROS.GUSTAN G
        INNER JOIN PARRANDEROS.BEBIDAS B ON G.ID_BEBIDA = B.ID
        WHERE B.GRADO_ALCOHOL <= 9
    )
)
GROUP BY F.ID_BEBEDOR, BEB.NOMBRE
ORDER BY NUM_VISITAS DESC, NOMBRE ASC
